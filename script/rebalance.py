...
# Ini kode untuk rebalance portfolio

def calculate_rebalancing_trades(
    # Data saldo awal dan harga token
    usdc_data: dict,  # {"balance": float, "price": float, "contract": Contract}
    weth_data: dict,  # {"balance": float, "price": float, "contract": Contract}
    target_allocations: dict[str, float],  # {"usdc": 0.3, "weth": 0.7} target alokasi portofolio
) -> dict[str, dict]:
    """
...
def get_price(feed_name: str) -> float:
    ...
    # diaz note - only when you need to do low level testing/changes to workaround moccasin bugs
    # config = get_config()
